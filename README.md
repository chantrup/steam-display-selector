...
- **Move the window after launch** — the renderer is already bound to the original monitor, causing the performance issues above.
...
**Steam Display Picker takes a different approach:** it edits the game's own config file *before* the game launches, so the renderer starts on the correct monitor from the very beginning. No primary-monitor switching, no after-the-fact window moving.
...
The script is a single `.bat` file. No installation, no background process, no extra software.
...
1. **Download** `SteamDisplayPicker.bat` and save it somewhere permanent, e.g. `C:\Users\YourName\Documents\SteamDisplayPicker.bat`.

2. **Add your game to the registry** inside the script (see [Adding a game](#adding-a-game) below). A few popular games are included as working examples.

3. **Set the Steam Launch Option** for your game:
   - Right-click the game in Steam → **Properties** → **General** → **Launch Options**
   - Enter: `"C:\Path\To\SteamDisplayPicker.bat" GAMEKEY %command%`
   - Replace the path with where you saved the file, and `GAMEKEY` with the key you defined in the registry.
...
Every game stores its display setting differently, so adding one means finding three things and adding a few lines to the **GAME REGISTRY** section near the top of the script.
...
### `string-multi-field`
For games (often RE Engine / Capcom titles) where one or more fields need to change when switching monitors — a monitor name, a display index, a resolution, or a combination. Each monitor gets one line listing all fields to update, separated by semicolons.
...
> The included games (Helldivers 2, Elden Ring Nightreign, Monster Hunter Wilds, Pragmata) use **one user's** specific config paths and monitor names as real-world examples. Replace those values with your own paths and monitor names.
...
These cover the three formats, so most new games can be added by following the closest matching example.
